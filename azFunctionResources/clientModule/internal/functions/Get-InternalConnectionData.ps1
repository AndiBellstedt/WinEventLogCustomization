...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'WinEventLogCustomization.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'WinEventLogCustomization.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'WinEventLogCustomization.Client.ProtectedToken'
