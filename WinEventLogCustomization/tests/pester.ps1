...
Remove-Module WinEventLogCustomization -ErrorAction Ignore
Import-Module "$testroot\..\WinEventLogCustomization.psd1"
Import-Module "$testroot\..\WinEventLogCustomization.psm1" -Force

# Need to import explicitly so we can use the configuration class
Import-Module Pester

Write-PSFMessage -Level Important -Message "Creating test result folder"
$null = New-Item -Path "$testroot\..\.." -Name TestResults -ItemType Directory -Force
...
if ($TestGeneral) {
    ...
    foreach ($file in (Get-ChildItem "$testroot\general" | Where-Object Name -like "*.Tests.ps1")) {
        if ($file.Name -notlike $Include) { continue }
        if ($file.Name -like $Exclude) { continue }

        Write-PSFMessage -Level Significant -Message "  Executing <c='em'>$($file.Name)</c>"
        $config.TestResult.OutputPath = Join-Path "$testroot\..\..\TestResults" "TEST-$($file.BaseName).xml"
...
if ($TestFunctions) {
    ...
    foreach ($file in (Get-ChildItem "$testroot\functions" -Recurse -File | Where-Object Name -like "*Tests.ps1")) {
        if ($file.Name -notlike $Include) { continue }
        if ($file.Name -like $Exclude) { continue }
