    ...
    process {
        ...
        foreach ($Path in $Paths) {
            if ($path) {
                ...
                if ($extension -notmatch '.xlsx$|.xlsm$') { throw "Import-Excel does not support reading this extension type $($extension)" }
                ...
                if ($Password) {
                    $ExcelPackage = New-Object -TypeName OfficeOpenXml.ExcelPackage -ArgumentList $Path, $Password
                } else {
                    $ExcelPackage = New-Object -TypeName OfficeOpenXml.ExcelPackage -ArgumentList $Path
                }
            }
            try {
                # Select worksheet
                ...
                foreach ($sheet in $Worksheet) {
                    ...
                    if ($DataOnly) {
                        ...
                        if ($NoHeader) {
                            $range = "A" + ($StartRow     ) + ":" + $endAddress
                        } else {
                            $range = "A" + ($StartRow + 1 ) + ":" + $endAddress
                        }
                        <#
                        ...
                        if ($StartColumn -gt $EndColumn) {
                            Write-Warning -Message "Selecting columns $StartColumn to $EndColumn might give odd results."
                        }

                        if ($NoHeader) {
                            $rows = $StartRow..$EndRow
                            if ($StartRow -gt $EndRow) {
                                Write-Warning -Message "Selecting rows $StartRow to $EndRow might give odd results."
                            }
                        } elseif ($HeaderName) {
                            $rows = $StartRow..$EndRow
                        } else {
                ...
                throw "Failed importing the Excel workbook '$Path' with worksheet '$WorksheetName': $_"; return
            } finally {
                if ($Path) {
