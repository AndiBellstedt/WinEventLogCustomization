...
$parentModule = 'WinEventLogCustomization'
if (-not $ModuleName) { $ModuleName = 'WinEventLogCustomization.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\WinEventLogCustomization'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\WinEventLogCustomization.psd1"
$prereqHash = @{
