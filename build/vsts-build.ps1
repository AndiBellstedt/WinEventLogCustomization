...
Copy-Item -Path "$($WorkingDirectory)\WinEventLogCustomization" -Destination $publishDir.FullName -Recurse -Force
...
$fileData = Get-Content -Path "$($publishDir.FullName)\WinEventLogCustomization\WinEventLogCustomization.psm1" -Raw
$fileData = $fileData.Replace('"<was not compiled>"', '"<was compiled>"')
$fileData = $fileData.Replace('"<compile code into here>"', ($text -join "`n`n"))
[System.IO.File]::WriteAllText("$($publishDir.FullName)\WinEventLogCustomization\WinEventLogCustomization.psm1", $fileData, [System.Text.Encoding]::UTF8)
